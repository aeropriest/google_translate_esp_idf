...
// #define GOOGLE_TRANSLATE_LANG_FROM "en-US"      //https://cloud.google.com/translate/docs/languages
// #define GOOGLE_TRANSLATE_LANG_TO "en-US"           //https://cloud.google.com/translate/docs/languages
...
#define GOOGLE_SR_LANG "en-IN" //"cmn-Hans-CN"            // https://cloud.google.com/speech-to-text/docs/languages
#define GOOGLE_TRANSLATE_LANG_FROM "en-IN" //"zh-CN"      //https://cloud.google.com/translate/docs/languages
#define GOOGLE_TRANSLATE_LANG_TO "en-US"           //https://cloud.google.com/translate/docs/languages
...
char fake_translated_text[] = "Are you ok?";
...
void translate_task(void *pv)
{
    ...
    while (1) {
        ...
        //seems like wrongly defined value?
        if (msg.source_type != 1048588) {
            ESP_LOGI(TAG, "[ * ] %d Source type [%d] %d is not a button, continue",  (int)msg.source_type, PERIPH_ID_BUTTON, (int)msg.data);
            continue;
        }

        // It's MODE button
        if ((int)msg.data == get_input_mode_id()) {
            ESP_LOGI(TAG, "[ * ] Mode button %d was pressed, exit now", (int)msg.data);
            break;
        }

        if ((int)msg.data != get_input_rec_id()) {
            ESP_LOGI(TAG, "[ * ] Pressed button %d was not record, should be %d", (int)msg.data, get_input_rec_id());
            continue;
        }
#define RECOGNIZE_ORIGINAL_SPEECH

        if (msg.cmd == PERIPH_BUTTON_PRESSED) {
            google_tts_stop(tts);
            ESP_LOGI(TAG, "[ * ] Resuming pipeline");
#ifdef RECOGNIZE_ORIGINAL_SPEECH
            google_sr_start(sr);
#endif
...
void app_main(void)
{
    ...
    xTaskCreate(translate_task, "gg_translation", 4 * 4096, NULL, 5, NULL);    
}
